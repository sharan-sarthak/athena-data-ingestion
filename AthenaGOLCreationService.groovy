...
@Transactional
class AthenaGOLCreationService extends BaseService {

    private static Logger log = Logger.getLogger("com.mphrx.AthenaGOLCreationService");
    ...
    public void callJobProcessing(JobConfiguration jobConfig) {
        ...
        for(def createHl7ForSourceAndPracticeId : createHl7ForSourceAndPracticeIdMap.entrySet()) {
            if(createHl7ForSourceAndPracticeId?.value?.size()>0) {
                ...
                log.info("Going to create GOL message with PracticeId : "+createHl7ForSourceAndPracticeId.key +" sourceOfOrigin : "+createHl7ForSourceAndPracticeId.value)
                requestMap.put("hl7Type", "GOL")
                requestMap.put("eventType","PC6")
                requestMap.put("referenceApiList", ["goalClinicalReference"])

...
@Transactional
class AthenaADTCreationService extends BaseService {

    private static Logger log = Logger.getLogger("com.mphrx.AthenaADTCreationService");
    ...
    public void callJobProcessing(JobConfiguration jobConfig) {
        ...
        for(def createHl7ForSourceAndPracticeId : createHl7ForSourceAndPracticeIdMap.entrySet()) {
            if(createHl7ForSourceAndPracticeId?.value?.size()>0) {
                ...
                requestMap.put("eventType", "A08")

                log.info("Going to create ADT message excluding AllergyIntolerance with PracticeId : "+createHl7ForSourceAndPracticeId.key +" sourceOfOrigin : "+createHl7ForSourceAndPracticeId.value)
                requestMap.put("hl7Type", "ADT")
                requestMap.put("referenceApiList", ["conditionDiagnosisClinicalReference", "observationClinicalReference", "procedureClinicalReference"])
                athenaIngestionService.createReferenceHL7ForEncounterLevelData(requestMap)

                log.info("Going to create ADT message for AllergyIntolerance with PracticeId : "+ createHl7ForSourceAndPracticeId.key +" sourceOfOrigin : "+createHl7ForSourceAndPracticeId.value)
                requestMap.put("hl7Type", "ADT_ALG")
                requestMap.put("referenceApiList", ["allergyIntoleranceClinicalReference"])

...
@Transactional
class AthenaVXUCreationService extends BaseService {

    private static Logger log = Logger.getLogger("com.mphrx.AthenaVXUCreationService");
    ...
    public void callJobProcessing(JobConfiguration jobConfig) {
        ...
        for(def createHl7ForSourceAndPracticeId : createHl7ForSourceAndPracticeIdMap.entrySet()) {
            if (createHl7ForSourceAndPracticeId?.value?.size() > 0) {
                ...
                log.info("Going to create VXU message with PracticeId : "+createHl7ForSourceAndPracticeId.key +" sourceOfOrigin : "+createHl7ForSourceAndPracticeId.value)
                requestMap.put("hl7Type", "VXU")
                requestMap.put("referenceApiList", ["immunizationClinicalReference"])
                athenaService.createReferenceHL7ForEncounterLevelData(requestMap)

                requestMap.put("hl7Type", "VXU_NE")
                requestMap.put("referenceApiList", ["immunizationClinicalReference"])
                athenaService.createReferenceHL7ForPatientLevelData(requestMap)
            }else{
